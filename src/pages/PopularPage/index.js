...
class PopularPage extends Component {
    ...
    getPopularMovies = async () => {
        ...
        const url = `https://api.themoviedb.org/3/movie/popular?api_key=c95584a34ca2f6b53f8a88c1d9cbb28c&language=en-US&page=${currentPage}`
        const options = {
            headers: {
        ...
        if (response.ok){
            ...
            this.setState({apiStatus: apiStatusConstants.success, popularList: updatedData, totalPage: data.total_pages})
    ...
    renderSwitchControls = () => {
        const {apiStatus, popularList, currentPage} = this.state
        const {searchInput} = this.props
        let searchResult
        if (searchInput === ''){
            searchResult = popularList
        }
        else{
            searchResult = popularList.filter(movie => movie.title.toLowerCase() === searchInput.toLowerCase())
        }
