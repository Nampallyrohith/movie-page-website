...
class TopRatedPage extends Component {
    ...
    getTopMovies = async () => {
        ...
        const url = `https://api.themoviedb.org/3/movie/top_rated?api_key=c95584a34ca2f6b53f8a88c1d9cbb28c&language=en-US&page=${currentPage}`
        const options = {
            headers: {
        ...
        if (response.ok){
            ...
            this.setState({apiStatus: apiStatusConstants.success, topRatedList: updatedData})
    ...
    renderSwitchControls = () => {
        const {apiStatus, topRatedList, currentPage} = this.state
        const {searchInput} = this.props
        let searchResult
        if (searchInput === ''){
            searchResult = topRatedList
        }
        else{
            searchResult = topRatedList.filter(movie => movie.title.toLowerCase() === searchInput.toLowerCase())
        }
